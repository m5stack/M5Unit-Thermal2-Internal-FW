...
namespace m5
{
  ...
  public:
    ...
    bool readWords(uint16_t *readdata, size_t length, bool last_nack = false);

    bool transactionWrite(int addr, const uint8_t *writedata, uint8_t writelen, uint32_t freq);
    bool transactionRead(int addr, uint8_t *readdata, uint8_t readlen, uint32_t freq);
    bool transactionWriteRead(int addr, const uint8_t *writedata, uint8_t writelen, uint8_t *readdata, size_t readlen, uint32_t freq);
