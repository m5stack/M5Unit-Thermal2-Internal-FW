...
bool IRAM_ATTR addData(std::uint8_t data) {
    ...
    _calc_crc32 =
        (_calc_crc32 << 8) ^ _crc_table[((_calc_crc32 >> 24) ^ data) & 0xff];
...
//*
    if ((!info->finish &&
         (ESP_OK != esp_partition_erase_range(_partition, info->offset,
                                              SPI_FLASH_SEC_SIZE))) ||
        (ESP_OK != esp_partition_write(_partition, info->offset, info->buffer,
                                       info->len)) ||
...
/*/
for (uint32_t i = 0; i < UINT16_MAX<<6; ++i) { __asm__ __volatile__("nop"); }
info->status = write_info_t::status_t::ok;
//*/
    vTaskDelete(nullptr);
}

static bool IRAM_ATTR write(std::uint8_t* buf, std::size_t offset, std::size_t len,
                  bool finish) {
    write_info_t info;
    info.buffer = buf;
    info.offset = offset;
    ...
    xTaskCreatePinnedToCore(writeTask, "writeTask", 4096, &info, 2, nullptr, PRO_CPU_NUM);
    do { vTaskDelay(1); } while (info.status == write_info_t::status_t::none);
