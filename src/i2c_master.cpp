...
static void i2c_set_cmd(i2c_dev_t *dev, uint8_t index, uint8_t op_code,
                        uint8_t byte_num, bool ack_value = false) {
...
bool I2C_Master::restart(int i2c_addr, bool read, uint32_t freq) {
    ...
    if (_state == state_t::state_disconnect || _freq != freq) {
        ...
        uint32_t scl_high_period =
            std::max<uint32_t>(18, (period_total - 10) >> 1);
