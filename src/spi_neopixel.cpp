...
// gpio_matrix_out(32, spi_periph_signal[spi_host].spid_out, false, false); // for DEBUG
// gpio_matrix_out(33, spi_periph_signal[spi_host].spiclk_out, false, false);

    uint32_t clkdiv = FreqToClockDiv(getApbFrequency(), 2500000);
    WRITE_PERI_REG(SPI_CLOCK_REG(spi_port), clkdiv);
    WRITE_PERI_REG(SPI_USER_REG (spi_port), SPI_USR_MOSI | SPI_USR_MISO | SPI_DOUTDIN);
    ...
    WRITE_PERI_REG(SPI_MOSI_DLEN_REG(spi_port), (32 << 3) - 1); // 送信ビット数は末尾に無信号期間を含める
    memset(reinterpret_cast<void*>(SPI_W0_REG(spi_port)), 0, 64); // 送信バッファをゼロクリア
  ...
  void NeoPixel_Class::setColor(uint32_t rgb)
  {
    ...
    /// RGB から GRBに変換
    uint32_t grb = ((rgb >> 16) << 8) + (((rgb >> 8) & 0xFF) << 16) + (rgb & 0xFF);
...
/*
  void NeoPixel_Class::setColor(uint32_t rgb)
  {
    ...
    /// RGB から GRBに変換
    uint32_t bits = ((rgb >> 16) << 8) + (((rgb >> 8) & 0xFF) << 16) + (rgb & 0xFF);

    /// SPIの1Byte で NeoPixelの2bitを再現する。24bitぶん再現するので12Byte使用する。
    for (int i = 0; i < 12; ++i)
    {
